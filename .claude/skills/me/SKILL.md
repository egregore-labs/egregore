...
2. **Neo4j — update name + store previous name:**
   ```bash
...
3. **Neo4j — merge orphaned duplicate** (absorb any Person node with the new name that has no github):
   ```bash
...
4. **Supabase** (via API, non-blocking):
   ```bash
