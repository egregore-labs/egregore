...
// Prime Agent built-in slash commands (README Commands table) plus
// conservative extras for surfaces added around launch. An Egregore skill
// with a colliding name stays reachable via /skill:<name>.
const BUILTIN_COMMANDS = new Set([
  "login", "logout", "model", "effort", "scoped-models", "settings", "resume",
  "new", "clear", "name", "session", "traces", "usage", "tree", "fork",
  "clone", "compact", "copy", "btw", "side", "export", "share", "reload",
  "hotkeys", "changelog", "quit", "skill", "mcp",
  "refine", "autonomous", "trust", "import", "agents", "harness", "schedule",
  "goal", "memory",
  // Observed at runtime (not in the README command table); Prime warns and
  // skips conflicting extension commands in autocomplete, so any future
  // collision is visible and non-fatal — add it here to keep startup quiet.
  "update",
]);
const PRODUCT_COMMANDS = new Set(["activity", "view", "handoff", "scroll"]);
const PRIME_SPEC = { runtimeName: "Prime Agent", specPath: ".prime/agent/APPEND_SYSTEM.md" };
...
export default function egregore(pi) {
  ...
  const sendQuietWorkflow = (name, args, ctx, options = {}) => {
    ...
    const content = options.prompt || prompt(name, args, options.extra || "");
  ...
  pi.registerCommand("activity", {
    ...
    handler: async (_args, ctx) => {
      ...
      // file: one fast kernel read, never truncated, no reason to re-run the
      // time-capped data script or spelunk renderer sources.
      const cached = lastActivityJson();
      const focusPrompt = focus === "Dashboard"
        ? prompt("dashboard", "")
        : [
      ...
      sendQuietWorkflow(command, "", ctx, { prompt: focusPrompt, working: `Opening ${focus.toLowerCase()}…` });
    },
  });
  ...
  pi.registerCommand("handoff", {
    ...
    handler: async (args, ctx) => {
      ...
      sendQuietWorkflow("handoff", args, ctx, { prompt: handoffPrompt, working: "Preparing handoff…" });
    },
  });
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    try {
      const card = await renderGreeting(pi, root, ctx.signal, "prime-session-start.sh");
      lastSyncAt = Date.now();
  ...
  pi.on("session_shutdown", async (event, ctx) => {
    ...
    // shutdown is Prime Agent's equivalent of Claude Code's SessionEnd hook.
    if (event?.reason === "reload") return;
    try {
      await captureSessionEnd(pi, root, ctx.sessionManager.getSessionFile(), undefined, "prime");
    } catch {
      // Loss-prevention capture must never prevent the harness from exiting.
      // The attendant remains the fallback for interrupted or failed shutdowns.
    }
  });
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    // Prime Agent's default surface is the persistent IPython kernel: file
    // writes and shell escapes arrive as Python code, not bash tool calls.
    const ipythonCode = event.toolName === "ipython" ? String(event.input?.code || "") : "";
    const mutates = isFileWrite ||
      (event.toolName === "bash" && isMutatingBash(command)) ||
      (event.toolName === "ipython" && isMutatingIpython(ipythonCode, ctx.cwd));
    if (!mutates) return;
    ...
    if (event.toolName === "ipython" && hub && hub.top !== ctx.cwd &&
        !isMutatingIpython(ipythonCode, hub.top)) return;
    let operation = hub;
    if (isFileWrite) {
    ...
    if (event.toolName === "bash" || event.toolName === "ipython") {
      const probe = event.toolName === "bash"
        ? String(event.input?.command || "").trim()
        : ipythonCode.replace(/^\s*!/, "").trim();
      if (isConsentCommand(probe, branch)) return;
    }
