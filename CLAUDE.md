...
- **Only local paths are judged.** Text a command hands to another machine — an `ssh` remote script, a `docker`/`kubectl exec` payload, an `scp`/`rsync` `host:path` operand — is stripped before the soft-tier scan, because those paths do not exist on this machine. Local operands on the same line (`ssh -i ~/.ssh/key`, a redirect target, an `scp` source) are still checked, and the hard tier still scans the whole command.
- **When the hook asks for consent** (soft-tier block): do not retry yet, do not route around via Bash, and do not list remediation as prose. Call `AskUserQuestion` with exactly the options the hook's stderr names: "Allow {dir} for this session" (on approval run `bash bin/boundary.sh grant "{dir}"`, then retry) / "Always allow on this instance" (on approval run `bash bin/boundary.sh grant --always "{dir}"`, adding `--write` when the block was a write, then retry) / "Paste contents inline" / "Cancel". Never run a grant without the user's explicit approval in that exchange. A grant takes effect on the next tool call, not the next session. Session grants are cleared on session start.
- **Never hand-edit the grant files.** `bin/boundary.sh grant` is the only supported way to widen the soft tier: it is the one command the hook exempts from its own scan, it refuses hard-tier and locked paths, and it refreshes the cached boundary so the retry actually works. `bin/boundary.sh grants` shows what is currently in effect; `revoke` takes it back.
